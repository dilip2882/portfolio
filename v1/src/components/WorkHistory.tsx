...
export const WorkHistory = () => {
  return (
    <div className="mb-12">
      <Heading
        as="h2"
        className="font-black text-2xl md:text-2xl lg:text-2xl my-4 mb-4"
      >
        Work History
      </Heading>
      {timeline.map((item, index) => (
        <div
          className="flex md:flex-row flex-col space-x-10 my-5 relative"
          key={`timeline-${index}`}
        >
          <Paragraph className="min-w-[176px]">{item.date}</Paragraph>
          <div>
            <Heading
              as="h5"
